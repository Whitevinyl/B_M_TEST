...
// https://github.com/mohayonao/timbre.js/blob/master/src/modules/compressor.js
...
function CompressorIII() {
    ...
    this.delayBufferL = new Float32Array(MaxPreDelayFrames);
    this.delayBufferR = new Float32Array(MaxPreDelayFrames);
...
proto.setAttackTime = function(value) {
    this.attackTime = Math.max(0.001, value);
    this._attackFrames = this.attackTime * sampleRate;
};

proto.setReleaseTime = function(value) {
    this.releaseTime = Math.max(0.001, value);
    var releaseFrames = this.releaseTime * sampleRate;

    var satReleaseTime = 0.0025;
    this._satReleaseFrames = satReleaseTime * sampleRate;
...
proto.setPreDelayTime = function(preDelayTime) {
    this.preDelayTime = preDelayTime;
    var preDelayFrames = preDelayTime * sampleRate;
...
proto.setParams = function(dbThreshold, dbKnee, ratio) {
...
proto.kAtSlope = function(desiredSlope) {
...
proto.updateStaticCurveParameters = function(dbThreshold, dbKnee, ratio) {
...
proto.process = function(channel) {
    ...
    var nDivisions = channel[0].length / nDivisionFrames;
    var frameIndex = 0;
    ...
    for (var i = 0; i < nDivisions; ++i) {
        ...
        while (loopFrames--) {
            ...
            var absUndelayedSource = (channel[0][frameIndex] + channel[1][frameIndex]) * 0.5;
            delayBufferL[preDelayWriteIndex] = channel[0][frameIndex];
            delayBufferR[preDelayWriteIndex] = channel[1][frameIndex];
            ...
            channel[0][frameIndex] = delayBufferL[preDelayReadIndex] * totalGain;
            channel[1][frameIndex] = delayBufferR[preDelayReadIndex] * totalGain;
